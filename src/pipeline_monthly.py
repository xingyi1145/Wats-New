"""
Monthly Pipeline – Low Velocity Data

Runs scrapers for static, long-term data (clubs, design teams),
then generates their embeddings.

Schedule: once per month via cron or task scheduler.
"""
...
logger = logging.getLogger("pipeline_monthly")
...
def run_step(name: str, script_path: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=900,  # 15 minute timeout per step (Playwright can be slow)
        )
        ...
        logger.warning(f"✗ Timeout: {name} exceeded 900s limit")
        return False
...
def main():
    logger.info("=" * 60)
    logger.info("  MONTHLY PIPELINE – Low Velocity Data")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info(f"  MONTHLY PIPELINE COMPLETE: {passed}/{total} succeeded, {failed} failed.")
    logger.info("=" * 60)
